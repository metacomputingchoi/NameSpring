...
class NameAnalysisService(private val hanja2Stroke: Map<String, Int>) {
        ...
        for (stroke1 in StrokeConstants.MIN_STROKE..StrokeConstants.MAX_STROKE) {
            for (stroke2 in StrokeConstants.MIN_STROKE..StrokeConstants.MAX_STROKE) {
                ...
                val analysisDetails = mutableMapOf<String, Any>(
                    ...
                    "four_types_luck" to fourTypes.map { if (it < StrokeConstants.LUCK_ARRAY_SIZE) luck[it] else 0 },
                    "initial_score" to score,
                    "name_pn" to namePn,
                    "name_pn_sum" to namePn.sum()
                )

                if (namePn.sum() == StrokeConstants.NAME_PN_SUM_INVALID_ZERO || namePn.sum() == StrokeConstants.NAME_PN_SUM_INVALID_THREE) {
                    score = 0
                    analysisDetails["score_zero_reason"] = "name_pn_sum_is_0_or_3"
                }

                var nameElements = listOf(
                    (surHanjaStroke % StrokeConstants.ELEMENT_NORMALIZE_VALUE) + (surHanjaStroke % StrokeConstants.ELEMENT_NORMALIZE_VALUE) % 2,
                    (stroke1 % StrokeConstants.ELEMENT_NORMALIZE_VALUE) + (stroke1 % StrokeConstants.ELEMENT_NORMALIZE_VALUE) % 2,
                    (stroke2 % StrokeConstants.ELEMENT_NORMALIZE_VALUE) + (stroke2 % StrokeConstants.ELEMENT_NORMALIZE_VALUE) % 2
                ...
                for (k in StrokeConstants.ELEMENT_LOOP_START..StrokeConstants.ELEMENT_LOOP_END) {
                    ...
                    when (diff) {
                        StrokeConstants.ELEMENT_DIFF_CONFLICT_1, StrokeConstants.ELEMENT_DIFF_CONFLICT_2 -> {
                            score = 0
                            checkInfo["result"] = "conflicting"
                            analysisDetails["score_zero_reason"] = "name_elements_conflict_at_${k-1}_$k"
                        }
                        StrokeConstants.ELEMENT_DIFF_HARMONY_1, StrokeConstants.ELEMENT_DIFF_HARMONY_2 -> {
                            scoreCoexistName++
                ...
                var typeElements = fourTypes.map { ft ->
                    val te = (ft % StrokeConstants.ELEMENT_NORMALIZE_VALUE) + (ft % StrokeConstants.ELEMENT_NORMALIZE_VALUE) % 2
                ...
                if (score >= StrokeConstants.MIN_FINAL_SCORE) {
                    results.add(mapOf(
                        "stroke1" to stroke1,
                        "stroke2" to stroke2,
    ...
    fun mapToCombinationAnalysis(map: Map<*, *>): NameCombination {
