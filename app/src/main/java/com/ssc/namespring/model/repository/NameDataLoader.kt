// model/repository/NameDataLoader.kt
...
class NameDataLoader {
    ...
    fun loadOptimizedMapping(context: Context): NameData.OptimizedMapping? {
        return try {
            context.assets.open("name/name_optimized_search_mapping.json").use { stream ->
                BufferedReader(InputStreamReader(stream, "UTF-8")).use { reader ->
                    gson.fromJson(reader, NameData.OptimizedMapping::class.java).also {
    ...
    fun loadCharTripleDict(context: Context): Map<String, NameData.CharTripleInfo> {
        return try {
            context.assets.open("name/name_char_triple_dict_effective.json").use { stream ->
                BufferedReader(InputStreamReader(stream, "UTF-8")).use { reader ->
                    val type = object : TypeToken<Map<String, NameData.CharTripleInfo>>() {}.type
                    val dict: Map<String, NameData.CharTripleInfo> = gson.fromJson(reader, type)
