// model/domain/service/NameDataSearcher.kt
...
class NameDataSearcher {
    ...
    fun searchHanja(query: String, optimizedMapping: OptimizedMapping): List<HanjaSearchResult> {
    ...
    private fun searchByChosung(query: String, mapping: OptimizedMapping): List<HanjaInfo> {
        Log.d(TAG, "초성 검색 모드")
        return mapping.chosungToHanjaInfo[query] ?: emptyList()
    }

    private fun searchByKorean(query: String, mapping: OptimizedMapping): List<HanjaInfo> {
        Log.d(TAG, "한글 검색 모드")
        return mapping.koreanToHanjaInfo[query] ?: emptyList()
    }

    private fun searchByHanja(query: String, mapping: OptimizedMapping): List<HanjaInfo> {
        Log.d(TAG, "한자 검색 모드")
        return mapping.hanjaToHanjaInfo[query] ?: emptyList()
    }

    private fun searchByMeaning(query: String, mapping: OptimizedMapping): List<HanjaInfo> {
        Log.d(TAG, "뜻 검색 모드")
